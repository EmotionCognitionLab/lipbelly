...
function notifyAvgCoherence() {
    try {
        // we need a minimum of 4.5 minutes of data and we get
        // (ideally) two values per second
        const min_values = 2 * 60 * 4.5;
        if (coherenceValues.length < min_values) {
            logger.error(`Regime ${JSON.stringify(curRegime)} starting at ${curSegmentStartMsOffset} has ended but there are less than 4.5 minutes of data (${coherenceValues.length} coherence values). Unable to report average coherence.`);
            return;
        }
